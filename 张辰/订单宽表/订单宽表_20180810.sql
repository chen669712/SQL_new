...
partition(d = '${zdt.addDay(-1).format("yyyy-MM-dd")}')
    select a.orderid as orderid 
        ...
        , datediff(substr(a.departure,1,10),substr(a.arrival,1,10)) as ciiquantity
    ...
    left outer join 
    (select hotel,hotelname from Dim_HtlDB.dimhtlhotel) b
    on a.hotel = b.hotel
    ...
    select a.orderid as orderid
          ...
          , datediff(c.checkout,c.checkin) as ciiquantity
    ...
    from
      ...
      left outer join
      (select orderid,orderitemid,statusid,vendorid,productname from ods_htl_bnborderdb.order_item 
        where d = '${zdt.addDay(0).format("yyyy-MM-dd")}'
        and producttype = 1) b
...
select old.ctriporderid as orderid
      , substr(old.createtime,1,8) as ordertime
      , old.uid as uid
      , null as vid
      , null as clientid
      ...
      , datediff(old.checkout,old.checkin) as ciiquantity
  ...
  left outer join ods_htl_bnborderdb.order_header_v2 oi
  on old.ctriporderid = oi.orderid
